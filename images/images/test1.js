...
var prev =document.querySelector("#next"); // 다음 버튼
...
function prevSlide() {
    if (current > 0) current -= 1; //현재 이미지가 첫 번째가 아니라면 이전 위치로
...
function nextSlide() { 
    if (current < slides.length - 1) current += 1; //현재 이미지가 마지막이 아니라면 다음 위치로
